...
//	}
	...
	/**
	 * Due to the way the new firewall works, we need to reinitialize the firewall to create the new rules in the
	 * firewall.php file.
	 */
	if ( $prev_version && version_compare( $prev_version, '9.4.3', '<=' ) ) {
	...
	/**
	 * Standardize REST API namespaces to 'really-simple-security'
	 * Update stored CSP report URL to use the new namespace
	 */
	if ( $prev_version && version_compare( $prev_version, '9.5.0.1', '<=' ) ) {
		// Update the stored CSP report URL to use the new namespace
		$current_csp_url = get_option( 'rsssl_csp_report_url' );
		if ( $current_csp_url && strpos( $current_csp_url, 'rsssl/v1/csp' ) !== false ) {
			$new_csp_url = str_replace( 'rsssl/v1/csp', 'really-simple-security/v1/csp', $current_csp_url );
			update_option( 'rsssl_csp_report_url', $new_csp_url, false );
			do_action('rsssl_update_rules');
		}
	}

	// Drop deprecated event_name and description columns from rsssl_event_logs table.
	// Translations are now built dynamically at retrieval time using event_id. See NL14RSP1-544.
	if ( $prev_version && version_compare( $prev_version, '9.5.7', '<' ) ) {
		global $wpdb;
		$table = $wpdb->base_prefix . 'rsssl_event_logs';

		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) ) {
			// Check columns exist before dropping (for MySQL < 8.0.29 compatibility).
			$columns = $wpdb->get_col( "SHOW COLUMNS FROM `$table`" );
			if ( in_array( 'event_name', $columns, true ) ) {
				$wpdb->query( $wpdb->prepare( 'ALTER TABLE %i DROP COLUMN `event_name`', $table ) );
			}
			if ( in_array( 'description', $columns, true ) ) {
				$wpdb->query( $wpdb->prepare( 'ALTER TABLE %i DROP COLUMN `description`', $table ) );
			}
		}
	}

	if ( $prev_version && version_compare( $prev_version, '9.5.10', '<' ) ) {
		require_once rsssl_path . 'pro/security/wordpress/passkey/models/class-rsssl-webauthn.php';
		( new \RSSSL\Pro\Security\WordPress\Passkey\Models\Rsssl_Webauthn() )->migrate_legacy_trust_path_types();
	}
}
