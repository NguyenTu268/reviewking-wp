...
class rsssl_headers {
	...
	public function __construct()
	{
		...
		if ( ! defined('WPFC_SERVE_ONLY_VIA_CACHE') ) {
	...
	public function store_csp_endpoint(): void {
		if ( !get_option('rsssl_csp_report_url') ) {
			update_option('rsssl_csp_report_url', get_rest_url(null, 'really-simple-security/v1/csp'), false );
		}
	}
	...
	private function header_length_ok(){
		...
		if ( $headerTestStatusCache = $this->getHeaderTestStatusCache() ) {
			return $headerTestStatusCache === 'ok';
		}
		...
		if ( !is_wp_error($response) ) {
			...
			if ( $status >= 500 ) {
				$this->setHeaderTestStatusCache('error');
				return false;
			}
		...
		$this->setHeaderTestStatusCache('ok');
		return true;
	}

	/**
	 * Get header test status cache, cleanup if expired
	 * @return string|false Returns cached status or false if invalid/expired
	 */
	private function getHeaderTestStatusCache() {
		$expiry = get_option('rsssl_csp_header_test_status_expiry');
		$status = get_option('rsssl_csp_header_test_status');

		// Clean up if expired OR if we have status but no expiry (edge case)
		if ( ($expiry && time() > $expiry) || ($status && !$expiry) ) {
			delete_option('rsssl_csp_header_test_status');
			delete_option('rsssl_csp_header_test_status_expiry');
			return false;
		}
	...
	/**
	 * Set header test status cache with expiry
	 * @param string $status The status to cache ('ok' or 'error')
	 */
	private function setHeaderTestStatusCache( string $status ) {
		update_option('rsssl_csp_header_test_status', $status);
		update_option('rsssl_csp_header_test_status_expiry', time() + (5 * MINUTE_IN_SECONDS));
	}
	/**
