...
if ( ! class_exists( 'Rsssl_Event_Log' ) ) {
	...
	class Rsssl_Event_Log {
		 ...
		 *
		 * Note: event_name and description columns have been removed.
		 * Translations are built dynamically at retrieval time using event_id. See NL14RSP1-544.
		 *
		 * @return void
		 */
		...
		public function get_events( ?array $data = null ): array {
			...
			try {
				...
				foreach ( $result['data'] as $key => $value ) {
					...
					// Translate from event_id (works for all entries, old and new)
					// This ensures language switching works correctly and avoids text domain issues
					if ( isset( $value->event_id ) ) {
						$result['data'][ $key ]->event_name = \RSSSL\Pro\Security\WordPress\Eventlog\Rsssl_Event_Type::get_short_description_based_on_code( (string) $value->event_id );

						// Get additional data from event_data JSON for proper translation
						$event_data = array();
						if ( isset( $value->event_data ) && is_string( $value->event_data ) ) {
							$event_data = json_decode( $value->event_data, true );
							if ( json_last_error() !== JSON_ERROR_NONE ) {
								$event_data = array();
							}
						}
						$username     = $value->username ?? __( 'unknown', 'really-simple-ssl' );
						$ip           = $value->source_ip ?? null;
						$country      = $event_data['iso2_code'] ?? null;
						$country_name = $event_data['country_name'] ?? null; // Used for region events where iso2_code is empty. See NL14RSP1-544

						$result['data'][ $key ]->description = \RSSSL\Pro\Security\WordPress\Eventlog\Rsssl_Event_Type::get_description_based_on_code(
