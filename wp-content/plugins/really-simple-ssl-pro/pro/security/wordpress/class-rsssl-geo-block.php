...
use Exception;
use RSSSL\Pro\Security\WordPress\Firewall\Builders\Rsssl_Firewall_Rules_Builder;
use RSSSL\Pro\Security\WordPress\Firewall\Rsssl_Firewall_File_Handler;
...
class Rsssl_Geo_Block implements Rsssl_Country_Contract {
    ...
    /**
     * Integrating the new file handler.
     * @var Rsssl_Firewall_File_Handler
     */
    private Rsssl_Firewall_File_Handler $file_handler;
	...
	public function __construct() {
        $this->file_handler = new Rsssl_Firewall_File_Handler();
		...
		add_action( 'rsssl_daily_cron', array( $this, 'fetch_google_crawler_ips' ) );
    ...
    private static function deactivate(): void
    {
        ...
        // Removing firewall file. Instanced the class because of the static method.
        (new Rsssl_Firewall_File_Handler())->delete();
    }
	...
	public function execute_hooks(): void {
        ...
        add_action('rsssl_install_dependencies_cron', array($this, 'install_dependencies') );
	}
	...
	public function add_blocked_country( array $data ): array {
		...
		if ( ! $this->is_valid_country_code( $data['country_code'] ) ) {
			return $this->json_response( false, __( 'Invalid country code format.', 'really-simple-ssl' ) );
		}
		global $wpdb;
	...
	public function add_blocked_countries( array $data ): array {
		...
		foreach ( $data['country_codes'] as $country ) {
			...
			if ( ! $this->is_valid_country_code( $country['country_code'] ) ) {
				$errors[ $country['country_code'] ] = __( 'Invalid country code format.', 'really-simple-ssl' );
				continue;
			}
			global $wpdb;
			$table_name = sanitize_text_field( $wpdb->base_prefix . 'rsssl_geo_block' );
	...
	private function insert_country_to_table( string $table_name, array $data ): bool {
		...
		// If the country is the US. We executed the crawler ip check.
		if ( $data['country_code'] === 'US' ) {
			do_action( 'rsssl_update_rules' );
			$geo_block = self::get_instance();
     ...
     */
    public function generate_firewall_rules(): void {
        // If the firewall is disabled, do not add any rules.
        if ( ! rsssl_get_option('enable_firewall') ) {
            return;
        }

		$builder = (new Rsssl_Firewall_Rules_Builder());
		$builder->withPluginValidationSection()
				->withTranslatableStringsSection()
				->withBlockListSection()
				->withWhitelistSection()
				->withGeoDatabaseAndBlockPageSetting()
				->withCountryDetectionSection()
				//UserAgentSpecific
				->withUserAgentSection()
				// 404 file detection.
				->with404DetectionSection()
		;

		$sections = $builder->build('array');

		if ( $sections && $this->file_handler->create() ) {
			// Add the sections to the file handler.
			$this->file_handler->addSectionsByMarkers( $sections );
		}
    }
	...
	public function add_white_list_ip( array $data ): array {
		...
		if ( ! $ip_fetcher->is_valid_ip( $data['ip_address'] ) && ! $ip_fetcher->isCIDR( $data['ip_address'] ) ) {
			return $this->json_response( false, __( 'Invalid IP address format.', 'really-simple-ssl' ) );
		}
	 ...
	 * Prevents PHP code injection via malicious country codes.
	 *
	 * @param string $code The country code to validate.
	 * @return bool True if valid, false otherwise.
	 */
	public function is_valid_country_code( string $code ): bool {
		// ISO 3166-1 alpha-2 codes are exactly 2 uppercase letters
		return (bool) preg_match( '/^[A-Z]{2}$/', $code );
	}

	/**
	 * Fetches Google crawler IP ranges and stores them for later use
	 * Stores ranges in CIDR format to work with is_ip_address_in_range
	 */
	public function fetch_google_crawler_ips(): void {
		// If US in blocked regions
		$blocked_countries_array = $this->get_blocked_countries_list_to_array();
		if (! in_array( 'US', $blocked_countries_array, true ) ) {
			return;
		}

		// URLs for Google crawler IP ranges
		$google_ip_sources = array(
		...
		// Define storage path for IP data
		$storage_dir = wp_upload_dir()['basedir'] . '/really-simple-ssl/google-ip-data/';

		// Create directory if it doesn't exist
		if ( ! is_dir( $storage_dir ) && ! mkdir( $storage_dir, 0755, true ) && ! is_dir( $storage_dir ) ) {
			throw new RuntimeException( sprintf( 'Directory "%s" was not created', $storage_dir ) );
		}
		...
		foreach ($google_ip_sources as $source_name => $source_url) {
			// Fetch data
			...
			foreach ($data['prefixes'] as $prefix) {
				if (isset($prefix['ipv4Prefix'])) {
					// Store IPv4 prefixes directly in CIDR format
					$all_ip_ranges['cidr'][] = $prefix['ipv4Prefix'];
				} elseif (isset($prefix['ipv6Prefix'])) {
					// Store IPv6 prefixes in CIDR format
					$all_ip_ranges['cidr'][] = $prefix['ipv6Prefix'];
				}
			}

			// Store creation time if available
			if (isset($data['creationTime'])) {
				$creation_time = strtotime($data['creationTime']);
				if ($creation_time) {
					$all_ip_ranges['creation_times'][$source_name] = $creation_time;
				}
			}
		}

		// Save the consolidated data
		$ip_data_file = $storage_dir . '/google_crawler_ips.json';
		file_put_contents($ip_data_file, json_encode($all_ip_ranges));
	}
