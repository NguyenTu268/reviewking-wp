 ...
 * @website https://really-simple-ssl.com/about-us
 */
...
if ( ! class_exists( 'Rsssl_Limit_Login_Attempts' ) ) {
	...
	class Rsssl_Limit_Login_Attempts {
		 ...
		 *
		 * @param  string $identifier The username or email to check.
		 *
		 * @return string
		 */
		public function check_request_for_user( string $identifier ): string {
			$identifiers = $this->resolve_login_identifier( $identifier );
			return $this->check_against_users( $identifiers );
		}

		/**
		 * Resolves a login identifier (username or email) to an array of identifiers to check.
		 * If an email is provided, returns both the email and the corresponding username.
		 *
		 * @param  string $identifier The username or email to resolve.
		 *
		 * @return array Array of identifiers to check (username, or both email and username).
		 */
		private function resolve_login_identifier( string $identifier ): array {
			$identifiers = array( $identifier );

			// If an email address is provided, also include the corresponding username
		...
		public function check_against_users( array $usernames ): string {
			...
			if ( false === $allowlist_users ) {
				// phpcs:ignore WordPress.DB
				$allowlist_users = $wpdb->get_col( 'SELECT LOWER(attempt_value) FROM ' . $wpdb->base_prefix . "rsssl_login_attempts WHERE status = 'allowed' AND attempt_type = 'username' " );
				wp_cache_set( $cache_key_allowlist, $allowlist_users, null, self::CACHE_EXPIRATION );
			}

			if ( false === $blocklist_users ) {
				// phpcs:ignore WordPress.DB
				$blocklist_users = $wpdb->get_col( 'SELECT LOWER(attempt_value) FROM ' . $wpdb->base_prefix . "rsssl_login_attempts WHERE status = 'blocked' AND attempt_type = 'username' " );
			...
			foreach ( $usernames as $username ) {
				...
				if ( in_array( $lowercase_username, $allowlist_users, true ) ) {
					return 'allowed';
				}
				if ( in_array( $lowercase_username, $blocklist_users, true ) ) {
					return 'blocked';
				}
			}
